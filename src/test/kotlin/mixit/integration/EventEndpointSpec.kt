...
class EventEndpointSpec : SubjectSpek<Application>({
    ...
    describe("an event JSON endpoint") {
        ...
        it("should find mixit16") {
            subject.start()
            val response = webClient.exchange(GET("http://localhost:${subject.port}/api/mixit16").accept(APPLICATION_JSON_UTF8).build())

            StepVerifier.create(response.flatMap{ r -> r.bodyToMono(Event::class)})
                    .consumeNextWith { it->
